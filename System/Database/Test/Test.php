...
class Test {
    ...
    public static function runOne($dbname) {
        ...
        Query::withConnection($dbname, function() use (&$counter) {
        ...
        try {
            if ($this->result) {
                ...
                if (!$this->compare((array) $r, $this->result)) {
                    echo "wrong-result\n";
                    $self = $this;
                    self::withIndent(function() use ($self, $r){
                        echo "-----\n";
    ...
    private function withIndent($cb, $cols = 5, $prepend = '') {
        ...
        ob_get_clean();
        $res = explode("\n", $res);
        $res = array_map(function($line) use ($cols, $prepend) {
    ...
    private static function inc($file) {
        return require(dirname(__FILE__) . '/' . $file);
    }
}